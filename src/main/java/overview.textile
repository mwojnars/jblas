...
* Indexing is <em>zero</em>-based, both for linear access to elements 
  and for indexing of columns and rows.
...
* There exist only two-dimensional matrices. Vectors are matrices
  ...
  much more convenient than having separated classes.

* Every math operator maps to a short mnemonic name. For example, +
  becomes @add@, - becomes @sub@, * becomes @mul@, / becomes @div@,
  and so on.

* Often, you can pass a double or float value, or a matrix with only
